...
def train(opts):
    ...
    optimizer_D_A = torch.optim.Adam(D_A.parameters(), lr=opts.lr, betas=(opts.beta1,opts.beta2))
    optimizer_D_B = torch.optim.Adam(D_B.parameters(), lr=opts.lr, betas=(opts.beta1,opts.beta2))

    # transform
    transform = transforms.Compose([transforms.ToTensor(),
                                   transforms.Normalize((0.5,0.5,0.5), (0.5,0.5,0.5))])
    # dataloader
    train_dataloader = DataLoader(AugGANDataset(opts.dataset_name, transform), batch_size=opts.batch_size, shuffle=True, num_workers=opts.n_cpu, seg_class=opts.seg_class)
    val_dataloader = DataLoader(AugGANDataset(opts.dataset_name, transform, mode='val'), batch_size=5, shuffle=True, num_workers=1, seg_class=opts.seg_class)
    ...
    for epoch in range(opts.start_epoch, end_epoch):
        ...
        for index, batch in enumerate(train_dataloader):
            ...
            # total loss
            loss_mul = loss_gan + loss_cycle + loss_identity +loss_seg + loss_weight_sharing
            loss_mul.backward()
            optimizer_mul.step()
...
def save_sample(E_A, E_B, H_A, H_B, S_G_BA, S_G_AB, D_G_BA, D_G_AB, S_P_A, S_P_B, D_P_A, D_P_B, epoch, opts, val_dataloader):
    ...
    N, _, h, w = seg_A.shape
    seg_A = seg_A.reshape(N,opts.seg_class, -1).argmax(axis=1).reshape(N, 1, h, w).repeat(1, 3, 1, 1)*10
    seg_B = seg_B.reshape(N,opts.seg_class, -1).argmax(axis=1).reshape(N, 1, h, w).repeat(1, 3, 1, 1)*10
